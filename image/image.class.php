...
//-----------------------------------------------------------------------------
        ...
        public function __construct() {
            ...
            $args = func_get_args();
            if (count($args)==1) {
                if (!empty($args[0])) { $this->openImage($args[0]); }
            } elseif (count($args)==2) {
...
//-----------------------------------------------------------------------------
        ...
        public function attach($child) {
            ...
            if (!($child instanceof image_plugin)) { return false; }
...
//-----------------------------------------------------------------------------
        ...
        public function sendHeader($image_format=IMAGE_TYPE_PNG) {
            ...
            switch ($image_format) {
                ...
                case IMAGE_TYPE_GIF: header("Content-type: image/gif"); return true; break;
                case IMAGE_TYPE_PNG: header("Content-type: image/png"); return true; break;
                case IMAGE_TYPE_JPEG: header("Content-type: image/jpeg"); return true; break;
        ...
        public function imagePng($filename="") {
            ...
            if ($this->gd_support_png) {
                ...
                if (!is_array($e)) {
                    ...
                    echo $e['file']." at line ".$e['line'].": ".$e['message'];
...
//-----------------------------------------------------------------------------
        ...
        public static function hexColorToIntColor($intColor="000000") {
            ...
            $arrColor = image::hexColorToArrayColor($hexColor);
            $intColor = image::arrayColorToIntColor($arrColor);
...
//-----------------------------------------------------------------------------
        ...
        protected function __get($name) {
            ...
            if ($name == "image") { return $this->image; }

            if ($name == "handle_x") { return ($this->mid_handle==true) ? floor($this->imagesx()/2) : 0; }
            if ($name == "handle_y") { return ($this->mid_handle==true) ? floor($this->imagesy()/2) : 0; }
...
//-----------------------------------------------------------------------------
        ...
        private function _file_info($filename) {
            ...
            for ($i=0; $size > 1024 && $i < count($ext) - 1; $i++) { $size /= 1024; }
